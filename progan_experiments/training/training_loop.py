...
# from training.orojar import get_current_hessian_penalty_loss_weight
...
def training_schedule(
        ...
        G_lrate_base=0.0002,    # Learning rate for the generator.
        G_lrate_dict={},       # Resolution-specific overrides.
        D_lrate_base=0.0002,    # Learning rate for the discriminator.
...
def training_loop(
        ...
        image_snapshot_ticks=60,        # How often to export image snapshots?
        interp_snapshot_ticks=20,       # How often to generate interpolation visualizations in TensorBoard?
        network_snapshot_ticks=60,        # How often to export network snapshots?
        network_metric_ticks=60,        # How often to evaluate network snapshots on specified metrics?
    ...
    with tf.device('/gpu:0'):
        if resume_run_id is not None:
            ...
            # weight1 = G.get_var(weight_name)
            # U1, S1, V1 = np.linalg.svd(weight1, full_matrices=False)
            # new_weight1 = S1[:, None] * V1
            # G.set_var(weight_name, new_weight1)
        else:
