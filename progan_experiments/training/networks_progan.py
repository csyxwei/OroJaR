...
def G_paper(
    ...
    # print('Normalize Latents: %s' % normalize_latents)
        ...
        with tf.variable_scope('%dx%d' % (2**res, 2**res)):
            if res == 2:  # 4x4
                ...
                with tf.variable_scope('Dense'):
                    ...
                    x = tf.identity(x, name='Post_conv')
                    x = tf.reshape(x, [-1, nf(res - 1), 4, 4])
                    x = tf.identity(apply_bias(x), name='Post_bias')
                    # x = tf.identity(act(x), name='Post_act')
                    # x = tf.identity(PN(x), name='Post_norm')
                with tf.variable_scope('Conv'):
                    x = tf.identity(conv2d(x, fmaps=nf(res - 1), kernel=3, use_wscale=use_wscale), name='Post_conv')
                    x = tf.identity(apply_bias(x), name='Post_bias')
                    x = tf.identity(act(x), name='Post_act')
                    x = tf.identity(PN(x), name='Post_norm')
            else:  # 8x8 and up
                if fused_scale:
                    with tf.variable_scope('Conv0_up'):
                        x = tf.identity(upscale2d_conv2d(x, fmaps=nf(res - 1), kernel=3, use_wscale=use_wscale),
                                        name='Post_conv')
                        x = tf.identity(apply_bias(x), name='Post_bias')
                        x = tf.identity(act(x), name='Post_act')
                        x = tf.identity(PN(x), name='Post_norm')
    ...
    if structure == 'recursive':
        def grow(x, res, lod):
            y = block(x, res)
            def img(): return upscale2d(torgb(y, res), 2**lod)
            if res > 2:
...
def D_paper(
        ...
        structure=None,         # 'linear' = human-readable, 'recursive' = efficient, None = select automatically
        is_template_graph=False,        # True = template graph constructed by the Network class, False = actual evaluation.
        infogan_nz=0,            # Number of z components that D's encoder head should try to reconstruct
        return_infogan=False,        # True = return InfoGAN head's output instead of D scores.
        **_kwargs):                         # Ignore unrecognized keyword args.
    ...
    def block(x, res):  # res = 2..resolution_log2
        with tf.variable_scope('%dx%d' % (2**res, 2**res)):
            if res >= 3:  # 8x8 and up
                ...
                if infogan_nz > 0:  # InfoGAN's Q encoder output head (we access this in loss.py using graph ops):
                    with tf.variable_scope('Q_Encoder'):
                        x_in = x
                        with tf.variable_scope('Conv'):
                            x_in = act(apply_bias(conv2d(x_in, fmaps=nf(res - 1), kernel=3, use_wscale=use_wscale)))
                        with tf.variable_scope('Dense0'):
                            x_in = act(apply_bias(dense(x_in, fmaps=nf(res - 2), use_wscale=use_wscale)))
                        with tf.variable_scope('Dense1'):
                            x_in = apply_bias(dense(x_in, fmaps=infogan_nz, gain=1, use_wscale=use_wscale))
                            x_enc = tf.identity(x_in, name='QEncoding')
                        if return_infogan:
                            return x_enc
    ...
    if structure == 'recursive':
        def grow(res, lod):
            def x(): return fromrgb(downscale2d(images_in, 2**lod), res)
            if lod > 0:
