"""Handles downloading pre-trained model checkpoints and datasets from Hessian Penalty."""
...
def find_dataset(name):
    ...
    base_dest = os.path.join(config.data_dir, name)
    if not os.path.exists(base_dest):
        print(f'Downloading f{name} dataset...')
        os.makedirs(os.path.join(config.data_dir, name), exist_ok=True)
