...
class GAN128Model(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.nz = nz = opt.nz
        self.netG = net.Generator128(nz=nz, nc_out=nc_out).to(self.device)

        if opt.isTrain:
            self.model_names += ['netD']
            self.netD = net.Discriminator128(nc_in=nc_out).to(self.device)
            ...
            self.optimizer_D = torch.optim.Adam(itertools.chain(self.netD.parameters()), lr=opt.lr,
                                                betas=(opt.beta1, 0.999))
            ...
            if opt.reg_type == 'vp':
                ...
                self.optimizer_C = torch.optim.Adam(itertools.chain(self.netC.parameters()), lr=opt.lr,
                                                    betas=(opt.beta1, 0.999))
